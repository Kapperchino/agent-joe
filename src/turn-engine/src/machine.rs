...
impl TurnMachine {
    ...
    pub fn transition(&mut self, event: impl Into<Event>) -> Vec<Effect> {
        ...
        self.state = match (std::mem::take(&mut self.state), event.into()) {
            ...
            (state, Event::Session(SessionEvent::StartWorker(request))) => {
...
impl Session {
    fn transition(&mut self, event: SessionEvent, effects: &mut Vec<Effect>) {
        match event {
            ...
            SessionEvent::StartWorker(request) => match (&self.state, &self.role) {
                (TurnState::Idle, SessionRole::Interactive) => {
                    self.role = SessionRole::Worker(request);
    ...
    fn provider_failed(
        ...
        match ProviderRetry::new(self.mode, &failure, turn.phase.attempt) {
            ProviderRetry::Retry { attempt } => {
                let previous = turn.phase.scope.clone();
        ...
        match event {
            ToolEvent::Started {
                ...
                let started = self
                    .state
                    .tools_mut(tag)
                    .and_then(|tools| tools.batch.start(operation));
                if started.is_some() {
            ...
            ToolEvent::Completed { operation, result } => {
                let completed = self
                    .state
                    .tools_mut(tag)
                    .and_then(|tools| tools.complete(operation, result, revision));
    ...
    fn stop<P: Into<CleanupWork>>(
        ...
        if let (CleanupWork::Provider(_), TurnOutcome::Cancelled | TurnOutcome::Failed(_)) =
            (&turn.phase.work, &turn.phase.outcome)
        {
    ...
    fn finish_turn(&mut self, turn: Turn<Cleanup>, effects: &mut Vec<Effect>) {
        ...
        match std::mem::replace(&mut self.role, SessionRole::Interactive) {
            SessionRole::Worker(request) => {
                ...
                effects.push(Effect::ReplyWorker { request, outcome });
                effects.push(Effect::StopActor);
            }
...
impl Shutdown {
    fn finish(self, effects: &mut Vec<Effect>) {
        ...
        if let SessionRole::Worker(request) = self.role {
            effects.push(Effect::ReplyWorker {
                request,
                outcome: WorkerOutcome::Failed(WorkerFailure::Stopped),
            });
